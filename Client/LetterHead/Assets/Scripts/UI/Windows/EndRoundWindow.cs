...
public class EndRoundWindow : WindowController
{
    ...
    internal IEnumerator GetLongWords(int minLength, List<string> existingWords, Action<List<string>> callback)
    {
        ...
        for (int i = 0; i < prefixCount; i++)
        {
            ...
            for (int index = 0; index < words.Count; index++)
            {
                ...
                if (canSpell && !existingWords.Contains(word.ToLower()) && !WordManager.Instance.IsBadWord(word))
                {
                    yield return new WaitForEndOfFrame();
                    longWords.Add(word);
    ...
    public void RematchClicked()
    {
        ...
        Srv.Instance.POST("Match/Invite", new Dictionary<string, string>() { { "userId", GameManager.Instance.OpponentUserId().ToString() }, { "scoringType", GameManager.Instance.MatchDetails.ScoringType.ToString() } },
            (s) =>
            {
